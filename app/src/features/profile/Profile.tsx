...
export function Profile() {
  ...
  if (activeView === 'badges') {
    ...
    return (
      <ModuleLayout
        ...
        rightContent={
          <button onClick={() => setActiveView('main')} className="w-9 h-9 rounded-xl bg-white/[0.06] border border-white/[0.08] flex items-center justify-center hover:bg-white/[0.1] transition-colors">
  ...
  // ================================================================
  // SUB-VIEW: Saved Cards
  // ================================================================
  if (activeView === 'saved') {
    return (
      <ModuleLayout
        title="Saved Cards"
        subtitle={`${starredCards.length} cards`}
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-5">
            ...
            <div className="relative p-5">
              ...
              <div className="flex items-start gap-4 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-sm font-medium ${learnerTitle.color} mt-0.5`}>
                    {learnerTitle.title}
                  </p>
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    {isConfigured && user && (
                      <span className="flex items-center gap-1 text-[11px] text-text-muted">
                        {syncError ? (
                          <CloudOff className="w-3 h-3 text-coral" />
                        ) : (
                          <Cloud className="w-3 h-3 text-sage" />
                        )}
                        {formatLastSync()}
                      </span>
                    )}
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  { icon: <Trophy className="w-4 h-4" />, value: userProgress.xp.toLocaleString(), label: 'XP', color: 'text-golden', bg: 'bg-golden/10' },
                  { icon: <Flame className="w-4 h-4" />, value: currentStreak, label: 'Streak', color: 'text-coral', bg: 'bg-coral/10' },
                  { icon: <BookOpen className="w-4 h-4" />, value: userProgress.lessonsCompleted.length, label: 'Lessons', color: 'text-sage', bg: 'bg-sage/10' },
                  { icon: <TrendingUp className="w-4 h-4" />, value: longestStreak || currentStreak, label: 'Best', color: 'text-sunrise', bg: 'bg-sunrise/10' },
                ].map((stat) => (
                      ...
                      <div
                        className={`w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-bold transition-all ${
                          active
                            ...
                            : 'bg-white/[0.03] text-text-muted/40 border border-white/[0.04]'
                        }`}
                      >
                        {orderedLabels[i]}
                      </div>
        ...
        <div className="fixed -left-[9999px] top-0" aria-hidden="true">
          ...
          >
            ...
            <div style={{ fontSize: 13, color: '#a3a3a3', marginBottom: 6 }}>
              {learnerTitle.title} &bull; Level {userProgress.level}
            </div>
              ...
              .map((cat) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className={`text-[11px] font-bold ${categoryTextColor[cat.color] || 'text-text-muted'}`}>
                      {cat.progress}%
                    </span>
                  </div>
                  <div className="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                  ...
                  <p className="text-[10px] text-text-muted mt-1">
                    {cat.completedLessons} / {cat.totalLessons} lessons across {cat.modulesCount} modules
                  </p>
                </div>
              ))}
          </div>
        ...
        <motion.div variants={itemVariants} className="space-y-2">
          ...
          <button
            onClick={() => setActiveView('badges')}
            className="w-full flex items-center justify-between p-4 rounded-2xl border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.04] transition-colors"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-sunrise/10 border border-sunrise/20 flex items-center justify-center">
                <Award className="w-5 h-5 text-sunrise" />
              </div>
              <div className="text-left">
                <p className="text-sm font-medium text-text-primary">Badges</p>
                <p className="text-[11px] text-text-muted">{unlockedBadges.length} of {BADGES.length} earned</p>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-golden/10 border border-golden/20 flex items-center justify-center">
                <Bookmark className="w-5 h-5 text-golden" />
              </div>
              <div className="text-left">
                <p className="text-sm font-medium text-text-primary">Saved Cards</p>
                <p className="text-[11px] text-text-muted">{starredCards.length} cards saved</p>
              </div>
            </div>
            <ChevronRight className="w-4 h-4 text-text-muted" />
          </button>
          ...
          {/* Visual Lab */}
          <Link to="/visualizations" className="block">
            <div className="w-full flex items-center justify-between p-4 rounded-2xl border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-sm font-medium text-text-primary">Visual Lab</p>
                  <p className="text-[11px] text-text-muted">Interactive visualizations</p>
                </div>
              </div>
              <ChevronRight className="w-4 h-4 text-text-muted" />
            </div>
