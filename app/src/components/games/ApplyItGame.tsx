...
export function ApplyItGame({ content, quickPlay, onComplete, onUseHint, hintsRemaining }: GameComponentProps) {
  ...
  return (
    <div className="flex flex-col">
      {/* Minimal counter + hint — no redundant header */}
      <div className="flex items-center justify-between mb-3">
        <div className="px-2.5 py-1 rounded-full bg-white/[0.06] border border-white/[0.08]">
          <span className="text-xs font-medium text-text-muted">{currentIndex + 1}/{scenarios.length}</span>
        </div>
            ...
            {current.options.map((option, i) => {
              ...
              if (isEliminated && !showFeedback) {
                return (
                  <motion.div
                    ...
                    className="p-3 rounded-xl text-left text-sm border bg-white/[0.01] border-white/[0.03] line-through"
                  >
              ...
              return (
                <motion.button
                  ...
                  className={`w-full p-3 rounded-xl text-left text-sm border transition-all duration-200 ${
                    showCorrectness
                      ? isCorrect
                      ...
                      : 'bg-elevated/60 border-white/[0.08] hover:border-golden/30 hover:bg-golden/5 active:bg-golden/8'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${
                      showCorrectness
                        ...
                        : 'bg-white/[0.06] border-white/[0.10] text-text-secondary'
                    }`}>
