      ...
      <div className="relative z-10 p-5">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-full max-w-xs space-y-2">
            ...
            {contentTypes.map((content, idx) => (
              <motion.button
                ...
                className={`relative w-full overflow-hidden rounded-lg text-left transition-all`}
                whileTap={{ scale: 0.98 }}
              >
                <div className={`absolute inset-0 backdrop-blur-sm ${
                  activeType === idx ? 'bg-amber-500/10' : 'bg-[var(--viz-tile)]'
                }`} />
                <div className={`absolute inset-0 border rounded-lg transition-colors ${
                  activeType === idx ? 'border-amber-500/50' : 'border-[var(--viz-border)] hover:border-[var(--viz-border-light)]'
                }`} />
          ...
          <button
            ...
            className="text-xs text-purple-400 underline hover:text-purple-300 transition-colors"
          >
