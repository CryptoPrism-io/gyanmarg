#!/usr/bin/env python3
"""Fix navigation flows - extend SVG height and add at bottom without overlap"""
...
def fix_navigation_in_file(filepath, entries, exits, theme='dark'):
    """Remove old navigation and add new one with extended canvas"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Remove any existing navigation flow section
    content = re.sub(r'  <!-- Navigation Flow -->.*?(?=\n</svg>)', '', content, flags=re.DOTALL)

    # Get current viewBox and height
    viewbox_match = re.search(r'viewBox="0 0 (\d+) (\d+)"', content)
    if viewbox_match:
        original_height = int(viewbox_match.group(2))
    else:
    ...
    new_height = original_height + nav_section_height + 40  # 40px padding

    # Update viewBox and height
    content = re.sub(r'viewBox="0 0 1080 \d+"', f'viewBox="0 0 1080 {new_height}"', content)
    ...
    nav_section, _ = create_navigation_section(entries, exits, original_height + 20, theme)
    content += nav_section
...
print('Fixing navigation flows in dark theme pages...\n')
for filename, flows in navigation_flows.items():
    filepath = f'pages/{filename}'
    if os.path.exists(filepath):
        fix_navigation_in_file(filepath, flows['entry'], flows['exit'], theme='dark')
        print(f'✓ Fixed {filename}')

print('\n' + '='*60)
print('Fixing navigation flows in light theme pages...\n')

# Process light theme files
for filename, flows in navigation_flows.items():
    filepath = f'pages-bw/{filename}'
    if os.path.exists(filepath):
        fix_navigation_in_file(filepath, flows['entry'], flows['exit'], theme='light')
        print(f'✓ Fixed {filename}')

print('\n✅ Done! All 32 files fixed with extended canvas')
print('   Navigation flows now at bottom without overlap!')
